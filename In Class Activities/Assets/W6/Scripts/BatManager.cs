...
public class BatManager : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            BatW6 bat = _bats[i];
            float distance_bat_and_player = Vector2.Distance(bat.transform.position, _playerTransform.position);

            if (distance_bat_and_player < _interactDistance)
            {
                bat.Start_Chase(_playerTransform);
            }
            else
            {
                bat.Stop_Chase();
            ...
            //
            // STEP 4
            if (distance_bat_and_player < _overlapDistance)
            {
                CreateReactions(bat);
            }
    ...
    private void CreateReactions(BatW6 bat)
    {
        ...
        //      of CreateReactions.
        int random_index = UnityEngine.Random.Range(0, _messages.Length);
        string outputMessage = _messages[random_index];
        SpawnReactionUI(bat, outputMessage);
        // STEP 5 -------------------------------------------------------------
    }
